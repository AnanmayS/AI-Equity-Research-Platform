...
import { runTechnicalAnalysis } from "@/lib/ai";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await runTechnicalAnalysis(financialData);

    return NextResponse.json(result);
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Technical Analysis agent failed" },
      { status: 500 }
    );
  }
}
