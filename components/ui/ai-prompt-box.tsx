...
function PromptBoxStyles() {
  React.useEffect(() => {
    const styleSheet = document.createElement("style");
    styleSheet.dataset.promptBoxStyles = "true";
...
const PromptButton = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "default", size = "default", type = "button", ...props }, ref) => {
    ...
    const sizeClasses = {
      ...
      sm: "h-8 px-3 text-sm",
      lg: "h-12 px-6",
      icon: "h-8 w-8 aspect-[1/1] rounded-full"
    };
  ...
  const bars = React.useMemo(
    () =>
      Array.from({ length: visualizerBars }, (_, index) => ({
        ...
        delay: `${index * 0.05}s`,
        duration: `${0.5 + ((index * 13) % 50) / 100}s`
      })),
    [visualizerBars]
  );
    ...
    return (
      <>
        ...
        >
          ...
          <PromptInputActions
            className={cn(
              "flex items-center justify-between gap-2 p-0",
              simple ? "pt-0" : "pt-2"
            )}
          >
            {!simple ? (
            ...
            >
              <PromptButton
                ...
                onClick={() => {
                  ...
                  else if (!simple) setIsRecording(true);
                }}
              ...
              >
                {isLoading ? (
                  ...
                  <Mic className="h-5 w-5 text-[#D1D5DB] transition-colors" />
                )}
              </PromptButton>
            </PromptInputAction>
          </PromptInputActions>
